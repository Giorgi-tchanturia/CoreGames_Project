...
export const Home = () => {
    ...
    const fetchGames = async () => {
      ...
      try {
        const endpoint = searchQuery 
          ...
          : `https://api.rawg.io/api/games?key=${API_KEY}&ordering=-added&page_size=48`;
        ...
        if (searchQuery) {
          ...
          const discounts = data.slice(12, 24);

          const random = [...data.slice(24, 48)]
  ...
  const scrollSection = (sectionId, direction) => {
    const rail = sectionScrollRefs.current[sectionId];
    if (!rail) return;
    rail.scrollBy({ left: direction * Math.max(rail.clientWidth * 0.9, 280), behavior: 'smooth' });
  };
      ...
      {homeSections.map((section) => (
        <section key={section.id} className={styles.section}>
          ...
          <div className={styles.sectionHeader}>
            ...
            <div className={styles.carouselControls} aria-label={`${section.title} controls`}>
              <button type="button" className={styles.carouselButton} onClick={() => scrollSection(section.id, -1)} aria-label={`Previous ${section.title}`}>‹</button>
              <button type="button" className={styles.carouselButton} onClick={() => scrollSection(section.id, 1)} aria-label={`Next ${section.title}`}>›</button>
            </div>
          </div>

          <div ref={(element) => { sectionScrollRefs.current[section.id] = element; }} className={styles.cardsGrid}>
